    ...
    return Promise.all(
      ...
      .then(tables => {
        for (let i = 0; i < tables.length; i++) {
          ...
          const indexes = table.indexes.map(index => index.target.match(/(\w{1,})(?!\w*?\()/)[1])
          const atLeastOneIndexIncluded = indexes.reduce((acc, index) => {
            if (acc) return acc
            return propertiesInfo.map(p => p.columnName).includes(index)
          }, false)

          if (keysAreIncluded(table.partitionKeys, propertiesInfo) !== keyMatches.all && !atLeastOneIndexIncluded) {
